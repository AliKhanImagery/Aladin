...
import { X, Boxes, Trash2, RefreshCw, ArrowLeft, User, Package, MapPin, Clock } from 'lucide-react'
...
export default function MyAssetsPage() {
  ...
  const loadAssets = useCallback(async (silent = false) => {
    ...
    try {
      ...
      console.log('📦 Asset Bin: Loaded', data.length, 'items')
      setAssets(data || [])
    } catch (error: any) {
      console.error('❌ Asset Bin Error:', error)
        ...
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold tracking-tight italic serif">Assets</h1>
              <div className="px-2 py-0.5 rounded-md bg-white/5 border border-white/10">
                <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">{assets.length}</span>
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-8 py-12 relative z-10">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-32 text-white/20">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.4em]">Syncing Assets...</p>
          </div>
        ) : assets.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-40 text-center max-w-md mx-auto">
            <div className="w-20 h-20 bg-white/[0.02] border border:white/[0.05] rounded-[2rem] flex items-center justify-center mb-8">
              <Boxes className="w-8 h-8 text:white/10" />
            </div>
            <h2 className="text-2xl font-bold tracking-tight mb-4 italic serif text:white/60">No Assets Yet.</h2>
            <p className="text-sm text:white/20 font-medium leading-relaxed mb-10">
              Upload or generate assets from the Project Analysis screen. <br />
              All assets will appear here automatically.
            </p>
            <Link href="/">
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-xs text:white/30 line-clamp-2 leading-relaxed font-medium mb-4">
                      {asset.description || asset.prompt || 'No description provided.'}
                    </p>
      ...
      <footer className="py-16 px-8 border-t border:white/[0.03] mt-auto">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12 text:white/10">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em]">Assets v1.0.0</span>
          </div>
